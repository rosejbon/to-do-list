...
function App() {
  ...
  return (
    <main className="container mx-auto px-4">
      <h1 className="font-extrabold text-4xl py-8">
      ...
      {tasks.length === 0 ? (
        <div className="flex flex-col items-start justify-center py-8">
          <p className="mb-6 text-xl text-black max-w-xl">
            Either you have done everything already or there are still things
            for you to add to your list.
          </p>
          <Button asChild>
            <Link to="/create-task">Add your first to-do</Link>
          </Button>
        </div>
      ) : (
        <div
          className="overflow-auto w-full pb-4 focus:outline-2 focus:outline-indigo-600 focus:shadow-xl"
          role="region"
        ...
        >
          <table className="w-min md:w-full border-collapse border border-gray-300">
            <caption
              id="to-do-list"
              className="font-bold text-2xl py-4 text-left"
            >
              To-do list
            </caption>
            <thead>
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id}>
                  ...
                  <th className=" border-black border-2 px-4 py-2 bg-gray-100 ">
                    Edit
                  </th>
                  <th className=" border-black border-2 px-4 py-2 bg-gray-100">
                    Delete
              ...
              {table.getRowModel().rows.map((row) => (
                ...
                >
                  <Button asChild>
                    ...
                    >
                      Edit{' '}
                      <span className="sr-only">{row.original.name} task</span>
                    </Link>
                  </Button>
                </TaskRow>
              ))}
